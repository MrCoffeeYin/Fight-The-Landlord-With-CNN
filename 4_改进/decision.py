...
def main(full_input):
    # 准备数据
    full_input = json.loads(full_input)
    t0 = time.monotonic()
    ...
    def findPair(myCard):
        nonlocal counts, Pair, Triplet, Bomb
        for pair in Pair:
            result = []
            for card in myCard:
    ...
    def calculatePara(cards):
        nonlocal para1, para6, para7
        nonlocal myCard, cardRemaining, minlevel
        tempCard = copy.copy(myCard)
    ...
    if lastValid:   # 跟牌
        with open("../3_决策/data/theta1_1.csv", "r") as f:
            for i in range(0, theta1.shape[0]):
                line = f.readline().split(",")
                for j in range(0, len(line) - 1):
                    theta1[i][j] = float(line[j])
        with open("../3_决策/data/theta2_1.csv", "r") as f:
            for i in range(0, theta2.shape[0]):
                line = f.readline().split(",")
                for j in range(0, len(line) - 1):
                    theta2[i][j] = float(line[j])
        with open("../3_决策/data/theta3_1.csv", "r") as f:
            for i in range(0, theta3.shape[1]):
                theta3[0][i] = float(f.readline())
    else:   # 出牌
        with open("../3_决策/data/theta1_2.csv", "r") as f:
            for i in range(0, theta1.shape[0]):
                line = f.readline().split(",")
                for j in range(0, len(line) - 1):
                    theta1[i][j] = float(line[j])
        with open("../3_决策/data/theta2_2.csv", "r") as f:
            for i in range(0, theta2.shape[0]):
                line = f.readline().split(",")
                for j in range(0, len(line) - 1):
                    theta2[i][j] = float(line[j])
        with open("../3_决策/data/theta3_2.csv", "r") as f:
            for i in range(0, theta3.shape[1]):
                theta3[0][i] = float(f.readline())
...
if __name__ == '__main__':
    print(json.dumps({"response": (main(input()))}))
